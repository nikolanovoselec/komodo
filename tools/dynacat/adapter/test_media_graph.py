...
class MediaGraphTests(unittest.TestCase):
    def test_timestamp_gap_breaks_even_without_null_marker(self):
        result = graph([{'ts':1000,'rx_mbps':2,'tx_mbps':1},
                        {'ts':1040,'rx_mbps':4,'tx_mbps':2}], 1800, now=1040)
    ...
    def test_points_break_at_missing_not_zero(self):
        result=graph([{'ts':1000,'rx_mbps':2,'tx_mbps':1},{'ts':1005,'rx_mbps':None,'tx_mbps':None},{'ts':1010,'rx_mbps':4,'tx_mbps':2}],1800,now=1010)
        self.assertEqual(result['series'][0]['path'].count('M'),2)
        self.assertNotIn('L',result['series'][0]['path'])
    ...
    def test_real_contiguous_line(self):
        result=graph([{'ts':1000,'rx_mbps':0,'tx_mbps':1},{'ts':1005,'rx_mbps':1,'tx_mbps':2}],1800,now=1010)
