...
def test_total_series_paths_respect_gaps_and_thirty_minute_bounds(middle):
    ...
    h = pihole.query_history([{'history': records}, {'history': records}], 3000, window_seconds=1800)
    total = h['total']
...
def test_blocked_uses_independent_scale_and_permitted_retains_legacy_scale():
    ...
    h = pihole.query_history([first, second], 3000, window_seconds=1800)
...
def test_aligned_history_exact_sums_and_privacy():
    ...
    assert h['available'] and h['partial']  # Only 30 minutes of the requested day
...
def test_time_labels_and_absent_bin_gap():
    ...
    h=pihole.query_history([row,row],3000, window_seconds=1800)
    assert h['partial']
